...
## 金融机构共管账户 financial_managed_account_chaincode链码安装

# pee0-org1安装链码
docker exec -it cli-org1-peer0 bash

# 链码路径
export CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/chaincode/financial_managed_account_chaincode/main/
# 私有数据规则配置文件路径
...
# 打包链码
peer lifecycle chaincode package /usr/local/chaincode-artifacts/financial_managed_account_chaincode.tar.gz --path $CC_SRC_PATH --lang golang --label financial_managed_account_chaincode_1

# 安装链码
peer lifecycle chaincode install /usr/local/chaincode-artifacts/financial_managed_account_chaincode.tar.gz

# 将链码id设置变量,便于我们后面的使用
export CC_PACKAGE_ID=financial_managed_account_chaincode_1:1821cb30b13939e4bfbcdc4dbd7c8c1c0cc1b80bebb37f6ba7e3c0a7625f658f
...
# 查询默认公开数据
peer chaincode query -C $CHANNEL_NAME -n financial_managed_account_chaincode   -c '{"function":"FindById","Args":["F766005404604841984"]}'
# 查询默认私有数据
